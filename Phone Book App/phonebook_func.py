...
def center_window(self, w, h):  #  pass in the tkinter frame (master) reference and the w and h
    ...
    #   within our primary window we are able to get user's actual screenwidth
    screen_height = self.master.winfo_screenheight()
    #   within our primary window we are able to get user's actual screenheight
    #   calculate x and y coordinates to paint the app centered on the user's screen
    #   we fed values for w and h into the function when we called it in the main file
    x = int((screen_width/2) - (w/2)) # this gives us x coordinate of where to position form
    y = int((screen_height/2) - (h/2)) # this gives us y coordinate of where to position form
    centerGeo = self.master.geometry('{}x{}+{}+{}'.format(w, h, x, y)) # this gives dimensions to form for where to paint it
    return centerGeo
...
def onSelect(self,event):
    ...
    select = varList.cursorselection() [0]
    value = varList.get(selection)
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cursor = conn.cursor()
...
def addToList(self):
    ...
    var_fname = var_fname.title() # This will ensure that the first character in each word is capatilized
    var_lname = var_lname.strip()
    ...
    if (len(var_fname) > 0) and (len(var_lnam) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #enforce user to provide both names
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
            # Check the database for existance of the fullname, if so we will alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""", (var_fname,var_lname,var_fullname,var_phone,var_email))
            self.listList1.insert(END, var_fullname) #update listbox with new fullname
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            if count == 0 or count2 == 0:
                # if proposed changes are not already in database, then proceed
                response = messagebox.askokcancel("Update Request","The follwing changes({}) and ({}) will be implemented for ({}). \n\nProceed with the update request?".format(var_phone,var_email,var_value))
                print(response)
                if response:
                    conn = sqlite3.connect('db_phonebook.db')
                    with conn:
                    ...
                    messagebox.showinfo("Cancel_request","No changes have been made to({}).".format(var_value))
            else:
